...
export function ExportHistoryPanel({ refreshToken }: ExportHistoryPanelProps) {
  ...
  return (
    <div className={panelClass} style={panelStyle}>
      ...
      {detailLoading && (
        <div className="flex items-center justify-center gap-2 py-6 text-sm text-[#e8f8ff]/70">
      ...
      {!detailLoading && selected && (
        <div className="rounded-lg border border-[#e8f8ff]/15 bg-[#0a0c18a6] p-4">
          <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-[12px] text-[#e8f8ff]/60">
          ...
          <div className="mb-3 grid grid-cols-1 gap-x-4 gap-y-1.5 text-[13px] text-[#e8f8ff]/80 sm:grid-cols-2">
            <div>
              <span className="text-[#e8f8ff]/50">Договор с заказчиком: </span>
              {selected.contractName || '—'}
            </div>
            <div>
              <span className="text-[#e8f8ff]/50">Объект: </span>
              {selected.objectName || '—'}
            </div>
            <div>
              <span className="text-[#e8f8ff]/50">Отпустил: </span>
              {selected.releasedBy || '—'}
            </div>
            <div>
              <span className="text-[#e8f8ff]/50">Получил: </span>
              {selected.receivedBy || '—'}
            </div>
          </div>
              ...
              <tbody>
                {selected.items.map((item, i) => (
                  <tr key={i} className="hover:bg-white/4">
                    <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{item.nomenclatureName}</td>
                    <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{item.nomenclatureCode || '—'}</td>
                    <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{operationLabel(item.operationType)}</td>
                    <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{item.quantity}</td>
                    <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{item.unit || '—'}</td>
                    <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{item.person}</td>
                    <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{item.destination}</td>
                  </tr>
                ))}
          ...
          <div className="mt-4 flex justify-end gap-2 border-t border-[#e8f8ff]/10 pt-3.5">
            <button
              ...
              className="flex items-center gap-1.5 rounded-md border border-red-400/35 px-3 py-2 text-[12px] text-red-300 transition-colors hover:bg-red-500/15 disabled:opacity-50"
            >
      ...
      {!detailLoading && !selected && (
        <>
          {items.length === 0 && !loading ? (
            <div className="px-2.5 py-4 text-center text-[13px] text-[#e8f8ff]/50">Пока нет ни одного экспорта.</div>
          ) : (
                ...
                <tbody>
                  {items.map((exp) => (
                    <tr key={exp.id} className="cursor-pointer hover:bg-white/4" onClick={() => void openDetail(exp.id)}>
                      <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{exp.title}</td>
                      <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2 whitespace-nowrap">{formatDate(exp.createdAt)}</td>
                      <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{exp.createdBy}</td>
                      <td className="border-b border-[#e8f8ff]/8 px-2.5 py-2">{exp.itemsCount}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
