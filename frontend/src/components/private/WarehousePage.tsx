 ...
 * ведутся на портале. Доступ по правам роли (canManageWarehouseOperations/canSyncWarehouse1c/
 * canViewWarehouse), не по фиксированному admin — см. src/services/warehouse_service.py. */
      ...
      >
        ...
        {operations === null ? (
          ...
          <table className="w-full min-w-[820px] border-collapse text-[13px] text-[#e8f8ff]/85">
            <thead>
              <tr>
                {['Дата', 'Номенклатура', 'Тип', 'Кол-во', 'ФИО', 'Адрес/место назначения', 'Кто ввёл', ...(canManageOps ? ['Экспорт', 'Подтверждено в 1С', 'Действия'] : [])].map((h) => (
                  <th key={h} className="bg-white/6 px-2.5 py-2 text-left font-bold text-[var(--plasma-color)]">
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              ...
              {operations.length === 0 && (
                <tr>
                  <td colSpan={canManageOps ? 10 : 7} className="px-2.5 py-4 text-center text-[#e8f8ff]/50">
                    Операций пока нет.
                  </td>
                </tr>
              )}
