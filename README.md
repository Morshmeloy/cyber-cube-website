...
### Загрузка почтовых вложений

Почтовый запрос проходит два Nginx: публичный edge и WEB1. Лимит загрузки
должен быть достаточным на **каждом** слое; высокий лимит только на WEB1 не
поможет, если edge раньше вернёт HTTP 413.

14 сентября 2026 подтверждена причина ошибки SOGo при файле 1971 КБ:
публичный edge принимал 512 КиБ, но отклонял 2 МиБ до передачи на WEB1.
WEB1 тот же запрос принимал. Причиной оказался стандартный лимит Nginx `1m`,
а не формат RAR, имя файла или Mailcow.

Исправление: в TLS server-блоке `d4tech.ru` на публичном edge установить
`client_max_body_size 50m;`, проверить `nginx -t` и выполнить graceful
reload. Инструкция и приёмочные проверки:
[`deploy/nginx-mail-upload.conf.example`](deploy/nginx-mail-upload.conf.example).
