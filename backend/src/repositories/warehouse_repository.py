...
from src.models.warehouse import WarehouseItem, MovementType
...
class WarehouseRepository:
    ...
    async def create(
        self, item_data: WarehouseItemCreate, user_id: int
    ) -> WarehouseItem:
        item = WarehouseItem(
            name=item_data.name,
    ...
    async def get_all(self, skip: int = 0, limit: int = 100) -> List[WarehouseItem]:
        result = await self.db.execute(
            ...
            .order_by(WarehouseItem.date.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def update(
        self, item_id: int, update_data: WarehouseItemUpdate
    ...
    async def get_aggregated_balances(self) -> dict:
        ...
        for item in items:
            delta = (
                item.quantity
                if item.movement_type == MovementType.IN
                else -item.quantity
            )
